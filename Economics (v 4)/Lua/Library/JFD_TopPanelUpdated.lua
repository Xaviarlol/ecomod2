...
-- Globals
---------------------------------------------------------------------------------------------------------------------------
local IsGNK = ContentManager.IsActive("0E3751A1-F840-4e1b-9706-519BF484E59D", ContentType.GAMEPLAY)
...
function JFD_YieldAddinMetatable:YieldTooltipsNegative(tTooltips)
	...
	for _, entry in ipairs(self) do
		if entry.Yield < 0 then
			local toolTip = "[NEWLINE][ICON_BULLET]" .. L(entry.ToolTip, entry.Yield)
			if entry.YieldType == "YIELD_GOLD" then
				toolTip = "[NEWLINE]  [ICON_BULLET]" .. L(entry.ToolTip, math.abs(entry.Yield))--FA matching the formatting of the base TT
			end
			toolTipText = toolTipText .. toolTip
		end
	end
...
function UpdatedScienceTipHandler( control )
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
		...
		local iScienceFromTrade = 0
		if IsBNW then
			iScienceFromTrade = pPlayer:GetScienceFromCitiesTimes100(false) - iScienceFromCities;
		end
...
function UpdatedGoldTipHandler( control )
	...
	local fTradeRouteGold = 0 
	local fGoldPerTurnFromCities = pPlayer:GetGoldFromCitiesTimes100() / 100;
	if IsBNW then
		fTradeRouteGold = (pPlayer:GetGoldFromCitiesTimes100() - pPlayer:GetGoldFromCitiesMinusTradeRoutesTimes100()) / 100;
		fGoldPerTurnFromCities = pPlayer:GetGoldFromCitiesMinusTradeRoutesTimes100() / 100;
	end
	...
	local fTraitGold = 0
	if IsBNW then
		fTraitGold = pPlayer:GetGoldPerTurnFromTraits();
	end
	...
	if IsBNW then
		...
		strText = strText .. "[NEWLINE]  [ICON_BULLET]" .. L("TXT_KEY_TP_GOLD_FROM_TR", math.floor(fCityConnectionGold));
	end
...
function UpdatedHappinessTipHandler( control )
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
		...
		if IsExpansion then
			iCityHappiness = pPlayer:GetHappinessFromCities();
		end
		...
		if IsExpansion then
			iMinorCivHappiness = pPlayer:GetHappinessFromMinorCivs();
		end
		local iLeagueHappiness = 0
		if IsBNW then
			iLeagueHappiness = pPlayer:GetHappinessFromLeagues();
		end
		...
		local iUnhappinessPublicOpinion = 0
		if IsBNW then
			iUnhappinessPublicOpinion = pPlayer:GetUnhappinessFromPublicOpinion();
		end
...
function UpdatedCultureTipHandler( control )
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_POLICIES)) then
		...
		-- Culture from Traits
		local iCultureFromTraits = 0
		if IsBNW then
			iCultureFromTraits = pPlayer:GetJONSCulturePerTurnFromTraits();
		end
		...
		local iCultureFromMinors = 0
		if IsExpansion then
			iCultureFromMinors = pPlayer:GetCulturePerTurnFromMinorCivs();
		end
		...
		local iCultureFromReligion = 0
		if IsExpansion then
			iCultureFromReligion = pPlayer:GetCulturePerTurnFromReligion();
		end
		...
		local iCultureFromBonusTurns = 0
		if IsBNW then
			iCultureFromBonusTurns = pPlayer:GetCulturePerTurnFromBonusTurns();
		end
